/*
  # Rebuild ARISE Hunter System Database

  This migration rebuilds the entire database structure for the ARISE Hunter System.
  It creates all necessary tables, policies, and functions from scratch.
...
DROP TABLE IF EXISTS public.rule_daily_checks CASCADE;
...
CREATE INDEX idx_rule_violations_rule_id ON public.rule_violations(rule_id);
CREATE INDEX idx_rule_violations_date ON public.rule_violations(violation_date);

-- Create rule daily checks table
CREATE TABLE public.rule_daily_checks (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    rule_id UUID REFERENCES public.rules(id) ON DELETE CASCADE NOT NULL,
...
CREATE INDEX idx_rule_daily_checks_rule_id ON public.rule_daily_checks(rule_id);
CREATE INDEX idx_rule_daily_checks_date ON public.rule_daily_checks(check_date);
